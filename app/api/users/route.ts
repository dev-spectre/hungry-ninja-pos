...
export async function POST(req: Request) {
    try {
        ...
        const { name, username, password, role, branchId } = await req.json();
        ...
        if (r === "SHOP_MANAGER") {
            if (role === "SUPER_ADMIN" || role === "SHOP_MANAGER") {
        ...
        const user = await prisma.user.create({
            data: {
                 name, username, passwordHash: hashedPassword, role, branchId: targetBranchId
            }
        });
...
export async function DELETE(req: Request) {
    try {
        ...
        if (r === "SHOP_MANAGER" && (userToDelete.role === "SUPER_ADMIN" || userToDelete.role === "SHOP_MANAGER")) {
